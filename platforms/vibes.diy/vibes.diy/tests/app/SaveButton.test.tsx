...
describe("SaveButton", () => {
  ...
  describe("Rendering", () => {
    it("does not render if hasChanges is false", () => {
      const { queryByTestId } = render(<SaveButton onClick={mockOnClick} hasChanges={false} testId="save-button-1" />);
      expect(queryByTestId("save-button-1")).toBeNull();
    });

    it("renders button when hasChanges is true", () => {
      const { getByTestId } = render(<SaveButton onClick={mockOnClick} hasChanges={true} testId="save-button-2" />);
  ...
  describe("Props handling", () => {
    it("handles undefined syntaxErrorCount as 0", () => {
      const { getByTestId } = render(<SaveButton onClick={mockOnClick} hasChanges={true} testId="save-button-7" />);
    ...
    it("applies correct color variant when no errors", () => {
      const { getByTestId } = render(
        <SaveButton onClick={mockOnClick} hasChanges={true} syntaxErrorCount={0} color="retro" testId="save-button-8" />
      );
